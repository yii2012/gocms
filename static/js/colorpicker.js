 ...
 * @param showid 颜色面板id
 * @param fun 设置颜色的回调函数
 * @param handleId 被控制的的元素id (默认为title)
 * @param inputId 存储颜色值的id号
 */
function colorpicker(showid,fun,handleId,inputId) {
	for (i=0;i<2;i++) {
	  for (j=0;j<6;j++) {
	...
	colorTable='<div style="position:relative;width:253px; height:176px"><a href="javascript:;" onclick="closeBox();" class="close-own">X</a><table width=253 border="0" cellspacing="0" cellpadding="0" style="border:1px #000 solid;border-bottom:none;border-collapse: collapse" bordercolor="000000">'
			   ...
			   +'<td width="3"><td><input type="text" name="HexColor" size="7" id="input_colorId" style="border:inset 1px;font-family:Arial;" value="#000000"></td><td><a href="javascript:;" onclick="clear_title(\''+handleId+'\',\''+inputId+'\');"> clear</a></td></tr></table></td></table>'
...
/**
 * @param handleId 表单id号
 * @param inputId 存储颜色值的表单id
 */
function clear_title(handleId, inputId) {
	$('#' + handleId).css('color','');
	$('#' + inputId).val('');
