...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("队列文件数量超过设定值.\n" + (message === 0 ? "你已经达到了上传的限度." : "你最多能选择 " + (message > 1 ? message + " 个文件." : "一个文件.")));
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			progress.setStatus("文件尺寸超过设定值，最大上传文件大小为：" + file.size);
		...
		default:
			if (file !== null) {
				progress.setStatus("未知错误，请与管理员联系！");
			}
...
function uploadSuccess(file, serverData) {
	try {
		...
		//在页面中添加上传的文件信息
		handleUploadedPhoto(file, serverData);
	} catch (ex) {
		this.debug(ex);
	}
}
...
//上传成功后的处理函数
function handleUploadedPhoto(file, serverData) {
	$("#J_photoListContainer").append(serverData);
}